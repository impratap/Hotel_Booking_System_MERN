...
function Hero() {
  ...
  const {navigate, getToken, axios, setSearchedCiies} = useAppContext()
  ...
  const onSearch = async (e) => {
    ...
    // cal api to save recent searched city
    await axios.post('api/user/store-recent-search', {recentSearchedCity:
      destination}, {headers:{Authorization:`Bearer ${await getToken()}`}});

      // add destination to searchedCities max 3 recent searched citiesd

      setSearchedCiies((prevSearchedCities)=>{
  ...
  return (
    ...
    md:px-16 lg:px-24 xl:px-32 text-white bg-[url("/src/assets/heroImage.png")] bg-no-repeat bg-cover bg-center h-screen'>
      ...
      <p className='max-w-130 mt-2 text-sm md:text-base'>Unparalled luxury and comfort awiat at the world's most exclusive hotels and resorts.
