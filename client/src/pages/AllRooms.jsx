...
const CheckBox = ({label, selected = false, onChange = () => {}}) => {
    return (
        <label className='flex gap-34 items-center cursor-pointer mt-2 text-sm'>
...
function AllRooms() {
    ...
    const handleFilterChange = (checked, value, type) =>{
        setSelectedFilters((prevFilters)=>{
            const updatedFiters = {...prevFilters}
            if (checked) {
                updatedFiters[type].push(value);
            }
            else {
                updatedFiters[type] = updatedFiters[type].filter(item => item !== value);

            }

            return updatedFiters
        })
    ...
    // Function to check is a room matches the selected price ranges
        ...
        <div>
            <div className='flex flex-col items-start text-left'>
                ...
                <p className='text-sm md:text-base text-gray-500/90 mt-2 max-w-174'>Take advantage of our limited-time offers and special 
                    packages to enhnce your stay and create unforgettable memories
                </p>
            </div>
                    ...
                    <div className='md:w-1/2 flex flex-col gap-2'>
                        ...
                        <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                            {room.amenities.map((item, index)=>(
                                <div key={index} className='flex items-center gap-2 px-3 py-2 rouned-lg bg-[#F5F5FF]/70'>
        ...
        overflow-hidden transition-all duration-700`}>
            <div className='px-5 pt-5'>
                ...
                {roomTypes.map((room, index)=>(
                    ...
                    onChange={(checked)=>handleFilterChange
                        (checked, room, 'roomType')
                    }/>
                ))}
            </div>
